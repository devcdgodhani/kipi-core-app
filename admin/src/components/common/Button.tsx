import React from 'react';
import { Loader2 } from 'lucide-react';

interface CustomButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
    variant?: 'primary' | 'secondary' | 'accent' | 'outline' | 'ghost';
    fullWidth?: boolean;
    isLoading?: boolean;
    loading?: boolean; // alias for isLoading
}

const CustomButton: React.FC<CustomButtonProps> = ({
    children,
    variant = 'primary',
    fullWidth = false,
    isLoading = false,
    loading = false,
    ...props
}) => {
    const isActuallyLoading = isLoading || loading;
    const baseStyles = "px-6 py-3 rounded-lg font-bold transition-all duration-200 flex items-center justify-center gap-2";
    const variants = {
        primary: "bg-primary text-white hover:bg-primary/90 shadow-lg disabled:opacity-70",
        secondary: "bg-secondary text-white hover:bg-secondary/90 disabled:opacity-70",
        accent: "bg-accent text-white hover:bg-accent/90 shadow-lg hover:shadow-accent/25 disabled:opacity-70",
        outline: "border-2 border-primary text-primary hover:bg-primary/10 disabled:opacity-70",
        ghost: "bg-transparent text-gray-500 hover:bg-gray-100 border-none disabled:opacity-70"
    };

    return (
        <button
            {...props}
            disabled={props.disabled || isActuallyLoading}
            className={`${baseStyles} ${variants[variant]} ${fullWidth ? 'w-full' : ''} ${props.className || ''}`}
        >
            {isActuallyLoading && <Loader2 size={18} className="animate-spin" />}
            {children}
        </button>
    );
};

export default CustomButton;
