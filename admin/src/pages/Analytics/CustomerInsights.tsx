...
import { analyticsService, type ICustomerAnalytics } from '../../services/analyticsService';
...
const CustomerInsights: React.FC = () => {
    ...
    const fetchAnalytics = useCallback(async () => {
        try {
            setLoading(true);
            const cData = await analyticsService.getCustomerAnalytics(dateRange.startDate, dateRange.endDate);
    ...
    const handleExport = (format: 'xlsx' | 'csv' = 'xlsx') => {
        const url = `/api/v1/admin/analytics/export/customers?startDate=${dateRange.startDate.toISOString()}&endDate=${dateRange.endDate.toISOString()}&format=${format}`;
