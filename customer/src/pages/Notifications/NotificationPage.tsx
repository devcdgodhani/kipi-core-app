    ...
    return (
        <div className="max-w-4xl mx-auto pt-10 pb-20 px-4">
            <div className="flex items-center justify-between mb-10 border-b border-primary/10 pb-6">
                <div>
                    <h1 className="text-3xl font-black uppercase tracking-tight text-primary">Notifications</h1>
                    <p className="text-secondary text-sm mt-1">You have {unreadCount} unread messages</p>
                </div>
                {unreadCount > 0 && (
                    <button
            ...
            {(!notifications || notifications.length === 0) ? (
                <div className="py-20 text-center bg-primary/5 rounded-2xl border border-dashed border-primary/10">
                    ...
                    <p className="text-secondary font-medium">No notifications yet</p>
                </div>
            ) : (
                <div className="space-y-4">
                        ...
                        >
                            <div className={`w-12 h-12 flex-shrink-0 flex items-center justify-center rounded-xl ${notif.isRead ? 'bg-primary/5 text-secondary' : 'bg-background text-primary shadow-sm'
                                }`}>
                                <Bell size={20} />
                            </div>
                            ...
                            <div className="flex-1 space-y-1">
                                <div className="flex items-start justify-between">
                                    <h3 className={`text-sm font-bold uppercase tracking-tight ${notif.isRead ? 'text-secondary' : 'text-primary'}`}>
                                        {notif.title}
                                    </h3>
                                ...
                                <p className={`text-sm leading-relaxed ${notif.isRead ? 'text-secondary/70' : 'text-secondary'}`}>
                                    {notif.message}
                                </p>
