    ...
    return (
        <div className="bg-primary/5 p-6 rounded-2xl border border-primary/10 space-y-4">
            <div className="flex items-center gap-2 text-primary mb-2">
                ...
                <h3 className="text-sm font-bold uppercase tracking-widest text-primary">Delivery Options</h3>
            </div>
            ...
            <div className="flex gap-2">
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-3 bg-background border border-primary/20 rounded-xl text-sm focus:border-primary focus:outline-none transition-all uppercase font-medium text-primary"
                    />
                </div>
                <button
                    onClick={handleCheck}
                    disabled={loading || inputPincode.length !== 6}
                    className="px-6 py-3 bg-primary text-background text-xs font-black uppercase tracking-widest rounded-xl hover:bg-primary/90 transition-all disabled:opacity-50"
                >
            ...
            {etaData && etaData.length > 0 && (
                <div className="space-y-4 pt-2">
                    {etaData.map((eta, idx) => (
                        <div key={idx} className="bg-background p-4 rounded-xl border border-primary/10 shadow-sm animate-in fade-in slide-in-from-top-2 duration-300">
                            <div className="flex justify-between items-start mb-2">
                                <span className="text-xs font-bold text-primary uppercase">Express Delivery</span>
                            ...
                            <p className="text-sm font-medium text-secondary flex items-center gap-2">
                                Delivery by <span className="text-primary font-bold">{new Date(eta.estimatedDeliveryDate).toLocaleDateString(undefined, { weekday: 'short', day: 'numeric', month: 'short' })}</span>
                            </p>
                            <p className="text-[10px] text-secondary/60 mt-1 uppercase tracking-wider">
